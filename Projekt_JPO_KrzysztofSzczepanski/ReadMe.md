...
* **Decibel conversion:** Ability to convert values to a logarithmic scale for power, using the formula $10 \cdot \log_{10}(x)$. Conversion for voltage is also available based on the formula $20 \cdot \log_{10}(x)$. These functions work exclusively for positive numbers ($x > 0$).
...
To build the project, open the `build_x` directory, launch a terminal in that location, and type the following command:
`cmake ..`
The compiled, ready-to-run project files will be generated in this directory.

## User Manual
Upon launching the calculator, the main window is displayed, divided into two sections:
1. **Information Panel (Header):** Located at the top of the screen, it displays the currently selected numeral system (DEC by default) and the current operation result.
2. **Main Menu:** Contains a list of available options numbered from 1 to 9. To select an option, type the corresponding digit and press ENTER. After each operation, the screen is cleared and refreshed to display the latest result. 
